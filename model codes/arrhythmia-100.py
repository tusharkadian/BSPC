...
"""Load data"""
data_path = ''
x_train = np.load(data_path + 'x_train_100.npy')
y_train = np.load(data_path + 'y_train.npy')
x_test  = np.load(data_path + 'x_test_100.npy')
...
X = layers.Conv2D(filters=32, kernel_size=(1, 15))(input)
X = layers.BatchNormalization()(X)
X = layers.ReLU()(X)
X = layers.MaxPooling2D(pool_size=(1, 2), strides=1)(X)

convC1 = layers.Conv2D(filters=64, kernel_size=(1, 17))(X)

X = layers.Conv2D(filters=32, kernel_size=(1, 15))(X)
X = layers.BatchNormalization()(X)
X = layers.ReLU()(X)
X = layers.MaxPooling2D(pool_size=(1, 4), strides=1)(X)

convC2 = layers.Conv2D(filters=64, kernel_size=(1, 16))(convC1)

X = layers.Conv2D(filters=64, kernel_size=(1, 15))(X)
X = layers.BatchNormalization()(X)
X = layers.Add()([convC2, X])           # skip Connection
X = layers.ReLU()(X)
X = layers.MaxPooling2D(pool_size=(1, 2), strides=1)(X)

convE1 = layers.Conv2D(filters=32, kernel_size=(1, 14))(X)

X = layers.Conv2D(filters=64, kernel_size=(1, 13))(X)
X = layers.BatchNormalization()(X)
X = layers.ReLU()(X)
X = layers.MaxPooling2D(pool_size=(1, 4), strides=1)(X)

convE2 = layers.Conv2D(filters=64, kernel_size=(1, 15))(convE1)
...
X = layers.Dense(units=128, kernel_regularizer=regularizers.L2(0.009))(X)
X = layers.BatchNormalization()(X)
X = layers.ReLU()(X)
X = layers.Dropout(rate=0.2)(X)

X = layers.Dense(units=64, kernel_regularizer=regularizers.L2(0.015))(X)   # change to 0.001
X = layers.BatchNormalization()(X)
X = layers.ReLU()(X) 
...
model.compile(optimizer = optimizers.Adam(learning_rate=0.0005), 
              ...
              metrics = [metrics.BinaryAccuracy(), metrics.AUC(curve='ROC')])

history = model.fit(x_train, y_train, validation_split=0.10, epochs=40, batch_size=64, callbacks=callback)
...
save_path = ''
model.save(save_path + "finetuned-model-100.h5")
model.save(save_path + "finetuned-model-100")
